...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        # why temp[v]: if multiple edges within 1 iteration lead to v, it is okay to pick the best one as this is still 1 step (cascasding within the iteration allowed)
        # why dist[u]: this could lead to us finding a i + 1 step thats cheaper in the ith iteration, breaking the invariant
